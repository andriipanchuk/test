...
function bmt_remove_servicegroup_from_services($servicegroup, $services, &$msg)
{
    ...
    foreach ($services as $service_id) {
        ...
        exec_sql_query(DB_NAGIOSQL, "DELETE FROM `tbl_lnkServiceToServicegroup` WHERE `idMaster` = '$service_id' AND `idSlave` = '$servicegroup';", false);
        exec_sql_query(DB_NAGIOSQL, "UPDATE tbl_service SET `last_modified` = NOW() WHERE `id` = '$service_id';", true);
    }
...
function bmt_add_free_variables($hosts, $services, $contacts, $free_variable_names, $free_variable_values)
{
    ...
    if (!empty($hosts)) {
        foreach ($hosts as $host_id) {
            ...
            $free_variable_ids = exec_sql_query(DB_NAGIOSQL, "SELECT `tbl_variabledefinition`.`id` FROM tbl_variabledefinition INNER JOIN tbl_lnkHostToVariabledefinition ON `id` = `idSlave` WHERE `idMaster` = '$host_id' AND `name` in ('" . implode($free_variable_names, "','") . "');");
    ...
    if (!empty($services)) {
        foreach ($services as $service_id) {
            ...
            // Get free variables sharing our new names, delete those entries
            $free_variable_ids = exec_sql_query(DB_NAGIOSQL, "SELECT `tbl_variabledefinition`.`id` FROM tbl_variabledefinition INNER JOIN tbl_lnkServiceToVariabledefinition ON `id` = `idSlave` WHERE `idMaster` = '$service_id' AND `name` in ('" . implode($free_variable_names, "','") . "');");
            $free_variable_ids = $free_variable_ids->GetRows();
            ...
            foreach ($free_variable_names as $ind => $name) {
                ...
                exec_sql_query(DB_NAGIOSQL, "INSERT INTO tbl_variabledefinition (`name`, `value`, `last_modified`) VALUES ('" . $name . "','" . $free_variable_values[$ind] . "',NOW())", true);
    ...
    if (!empty($contacts)) {
        foreach ($contacts as $contact_id) {
            ...
            // Get free variables sharing our new names, delete those entries
            $free_variable_ids = exec_sql_query(DB_NAGIOSQL, "SELECT `tbl_variabledefinition`.`id` FROM tbl_variabledefinition INNER JOIN tbl_lnkContactToVariabledefinition ON `id` = `idSlave` WHERE `idMaster` = '$host_id' AND `name` in ('" . implode($free_variable_names, "','") . "');");
            $free_variable_ids = $free_variable_ids->GetRows();
