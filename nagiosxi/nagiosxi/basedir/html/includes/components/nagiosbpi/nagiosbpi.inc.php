    ...
    if (!empty($objs) && is_writable($bpi_config)) {
        ...
        foreach ($objs as $id => $obj) {
            ...
            if ($obj['type'] == 'host') {
                ...
                $old_name = trim(str_replace($chars, '', $obj['old_name']));
                $new_name = trim(str_replace($chars, '', $obj['host_name']));
                $find = $old_name.';';
                $replace = $new_name.';';
                ...
                $old_name = trim(str_replace($chars, '', $obj['old_name']));
                $new_name = trim(str_replace($chars, '', $obj['service_description']));
                
                foreach ($obj['hosts'] as $host) {
                    $host = trim(str_replace($chars, '', $host));
                    $find = $host.';'.$old_name.';&';
