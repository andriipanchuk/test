...
function ccm_table($args, $returnContent=array(0, ''), $page=1)
{
    ...
    {   
        ...
        if (!$ccm_restricted) {
            $icons .= "<span class='action ccm-tt-bind' title='"._('View config')."' onclick=\"actionPic('download', '".str_replace("'", "\'", $cfg_name)."', '{$_SESSION['domain']}')\"><img src='images/detail.png' alt='Download'></span>";
        }

        $icons .= "<span class='action ccm-tt-bind' title='"._('Relationships')."' onclick=\"show_relationship_popup('{$type}', '".str_replace("'", "\'", $info_name)."', '{$id}', '".$_SESSION['token']."')\"><img src='images/information.png' alt='Info'></span>";

        if (ccm_has_access_for($type)) {
            $icons .= "<span class='action ccm-tt-bind' title='Delete' onclick=\"actionPic('delete', '{$id}', '".str_replace("'", "\'", $name)."')\"><img src='images/cross.png' alt='Delete'></span>";
        }
...
function process_desc_exceptions(&$desc, $type, $data, $args, $id)
{
    ...
    if ($type == 'serviceescalation' || $type == 'servicedependency') {
        ...
        if ($data[$args['keyDesc']] == 1) {
            ...
            $query = "SELECT `id`, `service_description`, `exclude` FROM `tbl_service` LEFT JOIN `{$table}` ON `id`=`idSlave` WHERE `idMaster`=".$id;
