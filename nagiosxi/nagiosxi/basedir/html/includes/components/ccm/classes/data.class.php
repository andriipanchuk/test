    ...
    function purgeRelations($intDataId, $strTableName, $arrRelations, &$intDataCount=1, &$arrData=array())
    {
        ...
        foreach ($arrRelations as $rel) {
            ...
            if ($arrFlags[3] == 1) {
                ...
                // Special relationship for hosts -> services that will delete the objects
                // if the objects do not have any linkage to other objects
                if ($strTableName == 'tbl_host' && $rel['tableName'] == "tbl_lnkServiceToHost") {

                    // Loop through the service relationships
                    $strSQL = "SELECT id, config_name, service_description, host_name, hostgroup_name, servicegroups FROM `".$rel['tableName']."` LEFT JOIN ".$rel['target']." ON id = idMaster WHERE `".$rel['fieldName']."`=".intval($intDataId);
                    $booReturn = $ccm->db->getDataArray($strSQL, $arrData, $intDataCount);
                    if ($intDataCount != 0) {
                        foreach ($arrData as $vardata) {
                            if ($vardata['host_name'] == 1) {
                                ...
                                $strSQL = "SELECT * FROM tbl_lnkServiceToHost WHERE idMaster = ".$vardata['id'];
                                ...
                                if ($intDataCountRels == 1 && $vardata['hostgroup_name'] == 0) {
                                    ...
                                    $strSQL = "DELETE FROM tbl_lnkServiceToHost WHERE idMaster = " . intval($vardata['id']) . " AND idSlave = " . intval($intDataId);
                                    $booReturn = $ccm->db->insertData($strSQL);
            ...
            for ($i = 0; $i < $intDataCount; $i++) {
                ...
                if ($intDataId == 0 || $intDataId == $arrData[$i]['id']) {
                    ...
                    for ($y = 1; $y <= $intDataCount; $y++) {
                        ...
                        $booReturn = $ccm->db->getFieldData("SELECT `id` FROM `".$strTableName."` WHERE `".$strKeyField."`='$strNewName'");
                    ...
                    foreach ($arrData[$i] AS $type => $value) {
                        if ($type != $strKeyField && $type != "active" && $type != "last_modified" && $type != "id") {
                            ...
                            if (($strTableName == "tbl_timeperiod") && ($type == "name")) $value = $value."_copy_$y";
