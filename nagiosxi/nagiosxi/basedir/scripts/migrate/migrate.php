...
$opts = getopt("a:u:p:Ceoch", array('help', 'clear', 'overwrite'));
...
run_migration_ansible($address, $username, $password, $overwrite);

function run_migration_ansible($address, $username, $password, $overwrite = 0)
{
    global $ccm;
    ...
    $replace = array($become, $username, $password, $migrate_core_role);
    ...
    if (!empty($nagios_cfg)) {
        $args .= '-f '.escapeshellarg($nagios_cfg);
    }
    ...
    $cmd = "cp ".get_root_dir()."/scripts/migrate/nagios_unbundler.py ".get_root_dir()."/tmp/migrate/nagios_unbundler.py";
    $x = exec($cmd, $output, $code);
    $cmd = "cd ".get_root_dir()."/tmp/migrate && tar xf nagiosbundle-*.tar.gz && python nagios_unbundler.py";
    $x = exec($cmd, $output, $code);
    if ($code === 0) {
